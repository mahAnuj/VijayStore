...
// Verify OTP
export async function verifyOTP(phoneNumber: string, otp: string): Promise<{ success: boolean; message: string; user?: any }> {
  try {
    // Clean expired OTPs
    cleanExpiredOTPs();
...
export const isPhoneAuthenticated: RequestHandler = async (req: any, res, next) => {
...
export const isPhoneAdmin: RequestHandler = async (req: any, res, next) => {
  const userId = req.session?.userId;
